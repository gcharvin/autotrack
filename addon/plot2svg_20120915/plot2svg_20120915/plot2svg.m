...
for i=length(axchild):-1:1
    ...
    elseif strcmp(get(axchild(i),'Type'),'image')
        ...
        pointcclass = class(pointc);
        % Bugfix proposed by Tom
        if strcmp(get(axchild(i),'CDataMapping'),'scaled')
            clim=get(ax,'CLim');
            %figure, imshow(pointc,[]);
        ...
        if ndims(pointc) == 3
            ...
            %figure, imshow(pointc,[]);
            imwrite(uint8(pointc),fullfile(PLOT2SVG_globals.basefilepath,filename),PLOT2SVG_globals.pixelfiletype);
        else
