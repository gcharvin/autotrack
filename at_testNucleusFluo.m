...
for i=1:length(ib)
 ...
 img=phy_loadTimeLapseImage(segmentation.position,fra,channel,'non retreat'); % channel 2
 warning off all;
 img=imresize(img,2);
 warning on all;
 %figure, imshow(img,[]);
 ...
 if display
[fluo npeaks]=at_measureNucleusFluo(nucleus,img,hdisplay);
 else
 [fluo npeaks]=at_measureNucleusFluo(nucleus,img);    
 end
...
figure, plot(fr,arr); figure; plot(fr, arr2,'Color','r');