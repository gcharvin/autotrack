function [hPropsPane,parameters,OK] = at_propertiesGUI(hParent, parameters,title,description)
...
OK=0; % returns 0 is cancel is pressed
    ...
    if numParameters > 1
        ...
        for field_idx = 1 : length(field_names);
            ...
            elseif ischar(value)
                ...
                elseif strfind(value,'input_file')
                    type = 'file';
                    value = java.io.File(value);
                elseif value(1)=='*'
                    type = 'password';
            ...
            % propsList.add(newProperty(parameters.(field_name), 'flag_prop_name',   'Flag value:',     isEditable, 'boolean',            'Turn this on if you want to make extra plots', @propUpdatedCallback));
...
okEnabled = 'on';
try propsArray = propsList.toArray() ;
catch, return; end
for propsIdx = 1 : length(propsArray)
